<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $strand = $_POST["strand"];
        // Store LRN in a session variable
        ...
        $query1 = "INSERT INTO names (lrn, fname, mname, lname, age, year_level, strand)
                VALUES (?, ?, ?, ?, ?, ?, ?)";

        $stmt1 = $conn->prepare($query1);
        $stmt1->bind_param("ssssiss", $lrn, $fname, $mname, $lname, $age, $year_level, $strand,);
        $stmt1->execute();
        $stmt1->close();
    ...
    $email_query = "SELECT email FROM accounts WHERE account_Id = ?";
    $stmt_email = $conn->prepare($email_query);
    $stmt_email->bind_param("s", $account_id); // Replace with the actual identifier from your system
    $stmt_email->execute();
    $stmt_email->bind_result($user_email);

    // Fetch the email
    if ($stmt_email->fetch()) {
        echo "Email fetched successfully: " . $user_email; // Add this debug statement
    ...
    $stmt_email->close();
        ...
        // If the form is not submitted, redirect to the enrollment page
